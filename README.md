...
## Included Checks
- `uefi-unchecked-status`: Enforces proper handling of all `EFI_STATUS` and `RETURN_STATUS` returns by warning about not using return status. (supports GNU blocks, function pointers, and casts).
...
- `uefi-trace-function`: Enforces `TRACE_FUNCTION();` as the first statement in every function for selected files(use uefi-trace-function.TargetFiles), if `uefi-trace-function.TargetFiles` isn't provided, assumes TRACE_FUNCTION() is needed for every file in the project.

## Build & Test(Linux)
```bash
make init WORKSPACE_DIR_V=/home/var4p/Documents/uefi_workspace/edk2/..  # generate flags for test cases
